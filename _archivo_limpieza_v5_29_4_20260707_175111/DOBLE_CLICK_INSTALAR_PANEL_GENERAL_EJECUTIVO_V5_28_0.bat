...
color 0A
title SEO LOCAL v5.28.0 - Panel General Ejecutivo

echo ================================================================
echo SEO LOCAL v5.28.0 - Panel General Ejecutivo
echo ================================================================
echo.

set "SCRIPT_DIR=%~dp0"
set "PROJECT_DIR="
set "LOG=%USERPROFILE%\Desktop\seo_local_v5_28_0_panel_general_ejecutivo.log"

echo SEO LOCAL v5.28.0 - %DATE% %TIME%> "%LOG%"
...
if not exist "%PROJECT_DIR%\_backup_v5_28_0" mkdir "%PROJECT_DIR%\_backup_v5_28_0" >nul 2>&1
copy /Y "%PROJECT_DIR%\src\components\DashboardPage.tsx" "%PROJECT_DIR%\_backup_v5_28_0\DashboardPage.tsx.bak" >nul 2>&1
copy /Y "%PROJECT_DIR%\src\services\adminApi.ts" "%PROJECT_DIR%\_backup_v5_28_0\adminApi.ts.bak" >nul 2>&1
copy /Y "%PROJECT_DIR%\backend\src\server.js" "%PROJECT_DIR%\_backup_v5_28_0\server.js.bak" >nul 2>&1

echo [3/9] Copiando frontend ejecutivo...
...
if errorlevel 1 (
  echo ERROR: No pude copiar frontend.
  echo Error copiando frontend>> "%LOG%"
  pause
  exit /b 1
)

echo [4/9] Copiando migracion 025...
if not exist "%PROJECT_DIR%\backend\migrations" mkdir "%PROJECT_DIR%\backend\migrations" >nul 2>&1
copy /Y "%SCRIPT_DIR%payload\backend\migrations\025_dashboard_executive_overview_metrics.sql" "%PROJECT_DIR%\backend\migrations\025_dashboard_executive_overview_metrics.sql" >nul
if errorlevel 1 (
  echo ERROR: No pude copiar migracion 025.
  echo Error copiando migracion 025>> "%LOG%"
  pause
  exit /b 1
)

echo [5/9] Parcheando endpoint ejecutivo en API...
set "SEO_PROJECT_ROOT=%PROJECT_DIR%"
set "SEO_INSTALLER_ROOT=%SCRIPT_DIR%"
set "SEO_LOG_PATH=%LOG%"
node "%SCRIPT_DIR%tools\patch_executive_overview_v5280.cjs"
if errorlevel 1 (
  echo ERROR: No pude parchear la API.
...
echo.
echo ================================================================
echo Panel General Ejecutivo instalado correctamente.
echo ================================================================
echo.
