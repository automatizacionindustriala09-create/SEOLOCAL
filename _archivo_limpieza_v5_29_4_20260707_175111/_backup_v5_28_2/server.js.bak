...
app.get('/api/v1/admin/reports/executive', requireDashboardUser, requirePermission('reports.read'), asyncRoute(async (request, response) => {
  ...
  const [kpiResult, performanceResult, planMixResult, cityResult, topAgencyResult, funnelResult, activityResult] = await Promise.all([
    pool.query(`
      SELECT
        (SELECT COUNT(*)::int FROM seo_local_agency_profile ap WHERE ap.status IN ('published','review') ${agencyWhere}) AS active_agencies,
    ...
    pool.query(`
      SELECT COALESCE(NULLIF(p.city,''), 'Sin ciudad') AS city, COUNT(*)::int AS count,
             COUNT(*) FILTER (WHERE ap.status IN ('published','review'))::int AS active_count
      FROM seo_local_agency_profile ap
      JOIN res_partner p ON p.id = ap.partner_id
      WHERE ap.status <> 'suspended' ${agencyWhere}
    ...
    pool.query(`
      SELECT ap.partner_id AS id, p.name, p.city, ap.status,
             CASE WHEN ap.status IN ('published','review') THEN 'Activo' WHEN ap.status='review' THEN 'En pausa' ELSE 'Inactivo' END AS status_label,
    ...
    pool.query(`
      SELECT id, action AS event_type, CASE WHEN action='install' THEN 'Instalación de módulo' WHEN action='repair' THEN 'Reparación aplicada' WHEN action='status' THEN 'Cambio de estado' WHEN action='update' THEN 'Actualización de registro' ELSE INITCAP(action) END AS title,
