    ...
    <section className="bg-[#f5f5f5] min-h-screen pb-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <article id="profile-overview" className="space-y-5 overflow-visible">
          <div className="relative overflow-hidden rounded-[32px] border border-gray-200 bg-[#071A2F] shadow-sm">
        ...
        <div className="mt-7 grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-7 items-start">
          <div className="space-y-7">
            <section id="profile-info" className="grid grid-cols-1 xl:grid-cols-2 gap-5 scroll-mt-24">
              <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-6 sm:p-8 min-h-full">
                ...
                <div className="mt-6 rounded-2xl border border-gray-200 bg-gray-50 p-5 sm:p-6">
                  ...
                  <p className="mt-4 text-sm sm:text-[15px] font-semibold text-gray-600 leading-8">
                    {profile.tagline} La agencia opera con una metodología orientada a resultados locales medibles: diagnóstico inicial, priorización técnica, optimización de Google Business Profile, control de reputación, señales NAP consistentes y reportes de avance para tomar decisiones comerciales por ubicación.
                  </p>
                </div>
                <div className="mt-5 grid grid-cols-1 md:grid-cols-3 xl:grid-cols-1 2xl:grid-cols-3 gap-4">
                  <div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-[10px] font-black uppercase text-gray-400">Enfoque</span><p className="mt-2 text-sm font-black text-[#333] leading-snug">{profile.focus}</p></div>
                  <div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-[10px] font-black uppercase text-gray-400">Industrias</span><p className="mt-2 text-sm font-black text-[#333] leading-snug">{profile.industries.join(', ')}</p></div>
                  <div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-[10px] font-black uppercase text-gray-400">Cliente ideal</span><p className="mt-2 text-sm font-black text-[#333] leading-snug">{profile.clientProfile}</p></div>
                </div>
                <div className="mt-5 flex flex-wrap gap-2">
                  {(profile.identityTags || []).map((tag) => (<span key={tag} className="rounded-lg bg-red-50 text-[#D32323] px-2.5 py-1.5 text-[10px] font-black uppercase">#{tag.replace(/\s+/g, '')}</span>))}
                </div>
              </div>
              <div id="profile-gbp" className="bg-white rounded-3xl border border-gray-200 shadow-sm p-6 sm:p-8 min-h-full scroll-mt-24">
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-2xl bg-white border border-gray-200 shadow-sm flex items-center justify-center text-lg font-black"><span className="text-[#4285F4]">G</span></div><div><h2 className="text-xl font-black text-[#333]">Google Business Profile</h2><p className="text-[10px] uppercase tracking-wider font-black text-gray-400">Ficha local portable y verificada</p></div></div>
                ...
                <div className="rounded-3xl border border-gray-200 bg-gray-50 p-4">
                  <div className="grid grid-cols-1 md:grid-cols-[150px_1fr] gap-4">
                    <img src={currentAgency.image} alt={businessName} className="h-36 w-full rounded-2xl object-cover border border-gray-200" />
                    <div className="min-w-0"><div className="flex items-center gap-2 flex-wrap"><h3 className="text-xl font-black text-[#333] truncate">{businessName}</h3><BadgeCheck className="w-4 h-4 text-[#0074E0] shrink-0" /></div><p className="mt-1 text-sm font-semibold text-gray-500">Agencia de marketing digital</p><div className="mt-2 flex flex-wrap items-center gap-2 text-sm font-bold text-gray-500"><strong className="text-[#333]">{currentAgency.rating.toFixed(1)}</strong>{starRow(currentAgency.rating, 'w-4 h-4')}<span>({currentAgency.reviewsCount} reseñas)</span></div><div className="mt-3 space-y-2 text-sm font-semibold text-gray-600"><div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-[#0074E0]" /> <span className="truncate">{businessLocation}</span></div><div className="flex items-center gap-2"><Clock3 className="w-4 h-4 text-emerald-500" /> <span className="text-emerald-600 font-bold">Abierto</span> · Cierra a las 18:00</div><div className="flex items-center gap-2"><Phone className="w-4 h-4 text-gray-500" /> {currentAgency.phone}</div></div></div>
                  </div>
                  <div className="mt-4 rounded-2xl border border-gray-200 bg-white h-36 relative overflow-hidden"><div className="absolute inset-0 opacity-70" style={{ backgroundImage: 'linear-gradient(#eef2f7 1px, transparent 1px), linear-gradient(90deg, #eef2f7 1px, transparent 1px)', backgroundSize: '24px 24px' }} /><div className="absolute inset-0 bg-gradient-to-br from-[#0074E0]/5 via-transparent to-[#D32323]/5" /><div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center"><div className="w-10 h-10 rounded-full bg-[#D32323] text-white flex items-center justify-center shadow-xl ring-4 ring-white"><MapPin className="w-5 h-5 fill-white" /></div><span className="mt-2 rounded-full bg-white px-3 py-1 text-[11px] font-black text-[#333] shadow">{currentAgency.city || currentAgency.location}</span></div></div>
                  <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3"><a href={`tel:${currentAgency.phone}`} className="rounded-xl border border-[#0074E0]/20 bg-blue-50 px-3 py-3 text-center text-xs font-black text-[#0074E0] hover:bg-blue-100">Llamar</a><button type="button" onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(businessLocation)}`, '_blank', 'noopener,noreferrer')} className="rounded-xl border border-gray-200 px-3 py-3 text-center text-xs font-black text-[#333] hover:border-[#0074E0]/40">Cómo llegar</button><button type="button" onClick={() => window.open(websiteHref, '_blank', 'noopener,noreferrer')} className="rounded-xl border border-gray-200 px-3 py-3 text-center text-xs font-black text-[#333] hover:border-[#0074E0]/40">Sitio web</button><button type="button" onClick={() => onRequestQuote(currentAgency)} className="rounded-xl bg-[#0074E0] hover:bg-[#005fc0] px-3 py-3 text-center text-xs font-black text-white">Cotizar</button></div>
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3"><div className="rounded-2xl border border-gray-200 bg-white p-4"><h3 className="text-sm font-black text-[#333]">Reseña destacada</h3><p className="mt-2 text-xs font-semibold text-gray-600 leading-relaxed line-clamp-3">“{payload.reviews[0]?.body || currentAgency.highlightReview}”</p></div><div className="rounded-2xl border border-gray-200 bg-white p-4"><h3 className="text-sm font-black text-[#333]">Pregunta frecuente</h3><p className="mt-2 text-xs font-semibold text-gray-600 leading-relaxed">¿Atienden negocios fuera de {currentAgency.city || currentAgency.location}? Sí, con soporte local y remoto.</p></div></div>
                </div>
              </div>
            </section>
