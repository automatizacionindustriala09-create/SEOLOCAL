...
set "PAYLOAD=%INSTALLER_DIR%_seo_local_v5_18_payload"
set "TARGET="
set "BACKUP_DIR="

echo ================================================================
echo  SEO LOCAL v5.18 - Instalador Fichas 13, 14, 15 y 16
echo ================================================================
echo.

echo [1/9] Detectando proyecto destino...
...
if not exist "%PAYLOAD%\src\components\ServiceDetailPage.tsx.txt" (
  ...
  echo Verifica que el ZIP fue extraido completo y que existe _seo_local_v5_18_payload.
  pause
  exit /b 1
)
...
set "BACKUP_DIR=%TARGET%_backup_v5_18_%RANDOM%"
echo.
echo [2/9] Creando respaldo de archivos actuales...
...
echo.
echo [3/9] Instalando paginas funcionales FUR 13-16...
...
if exist "%INSTALLER_DIR%README_V5_18_FICHAS_13_16.md" copy /Y "%INSTALLER_DIR%README_V5_18_FICHAS_13_16.md" "%TARGET%README_V5_18_FICHAS_13_16.md" >nul

echo.
echo [4/9] Verificando marcadores de instalacion...
...
echo.
echo [5/9] Limpiando payloads antiguos que pueden romper TypeScript...
for %%P in (
...
echo.
echo [6/9] Limpiando cache de Vite...
...
echo.
echo [7/9] Instalando / sincronizando dependencias...
cd /d "%TARGET%"
call npm install
if errorlevel 1 (
...
echo.
echo [8/9] Validando TypeScript...
call npm run lint
if errorlevel 1 (
  echo ERROR: La validacion TypeScript fallo. Revisa el respaldo en %BACKUP_DIR%.
  pause
  exit /b 1
)

echo.
echo [9/9] Generando build frontend...
call npm run build
if errorlevel 1 (
  echo ERROR: El build frontend fallo. Revisa el respaldo en %BACKUP_DIR%.
  pause
  exit /b 1
)
...
echo.
echo ================================================================
echo  Instalacion v5.18 completada correctamente.
echo ================================================================
