...
title SEO LOCAL v5.22.1 - Reparar Diseno Premium Perfil Agencia
color 0A

echo ================================================================
echo SEO LOCAL v5.22.1 - Reparar Diseno Premium Perfil de Agencia
echo ================================================================
echo.
...
set "PROJECT_ROOT="
set "LOG=%USERPROFILE%\Desktop\seo_local_v5_22_1_reparar_diseno_premium_perfil_agencia.log"

echo Log: %LOG%
echo SEO LOCAL v5.22.1 - %date% %time% > "%LOG%"
...
set "BACKUP_DIR=%PROJECT_ROOT%\_backups_v5_22_1_perfil_agencia"
...
echo [2/8] Respaldando archivo actual...
if not exist "%BACKUP_DIR%" mkdir "%BACKUP_DIR%" >nul 2>&1
copy /Y "%TARGET_FILE%" "%BACKUP_DIR%\AgencyProfilePage.before.v5.22.1.tsx" >> "%LOG%" 2>&1

echo [3/8] Instalando diseno premium corregido...
...
echo [4/8] Actualizando version frontend...
node "%SCRIPT_DIR%tools\update_package_version_v5221.cjs" "%PROJECT_ROOT%" >> "%LOG%" 2>&1 || (
  echo ADVERTENCIA: No se pudo actualizar version en package.json.
  echo Version package no actualizada.>> "%LOG%"
)

echo [5/8] Verificando TypeScript local solo como diagnostico...
cd /d "%PROJECT_ROOT%"
if exist "node_modules\.bin\tsc.cmd" (
  call npx tsc --noEmit >> "%LOG%" 2>&1
  if errorlevel 1 (
    echo ADVERTENCIA: TypeScript local detecto detalles, pero no se bloqueara la instalacion.
    echo Revisa el log si quieres ver el detalle.>> "%LOG%"
  ) else (
    echo TypeScript local OK.>> "%LOG%"
  )
) else (
  echo node_modules local no disponible; se omite verificacion TypeScript local.>> "%LOG%"
)

echo [6/8] Verificando Docker Compose...
set "COMPOSE_CMD=docker compose"
%COMPOSE_CMD% version >nul 2>&1 || set "COMPOSE_CMD=docker-compose"
%COMPOSE_CMD% version >> "%LOG%" 2>&1 || (
  echo ERROR: Docker Compose no esta disponible.
  pause
  exit /b 1
)

echo [7/8] Reconstruyendo frontend Docker...
if exist ".env.docker" (
...
echo [8/8] Actualizacion completada.
echo.
echo ================================================================
echo SEO LOCAL v5.22.1 instalado correctamente.
echo ================================================================
echo.
...
powershell -NoProfile -Command "Get-Content -Tail 60 '%LOG%'" 2>nul
pause
exit /b 1
