...
const logPath = (process.env.SEO_LOG_PATH || path.join(process.env.USERPROFILE || process.cwd(), 'Desktop', 'seo_local_v5_27_4_agencias_reales_imagenes_personal.log')).replace(/^"|"$/g, '');
...
log('Parche agencias/media v5.27.4 iniciado.');
...
// La cadena de reemplazo se construye por arreglo para evitar SyntaxError con backticks JSX/SQL.
...
if (updateRegex.test(server)) {
  ...
  log('WARN: no se encontró el UPDATE exacto de agencia. Se mantiene sin reemplazar ese bloque.');
}
...
if (createRegex.test(server)) {
  ...
  log('WARN: no se encontró el CREATE exacto de agencia. Se mantiene sin reemplazar ese bloque.');
}

write(serverPath, server);
log('Parche agencias/media v5.27.4 finalizado OK.');
