// AGENCY_PROFILE_PAGE_V5_20_0_MARKER
...
import { Agency, AgencyProfilePayload, AgencyReview } from '../types';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <div className="mt-7 grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-7 items-start">
          <div className="space-y-7">
            ...
            <section id="profile-services" className="bg-white rounded-3xl border border-gray-200 shadow-sm p-6 sm:p-8 scroll-mt-24">
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                {visibleServices.map((service) => (
                  <button key={service.id} type="button" className="text-left rounded-2xl border border-[#333]/50 hover:border-[#D32323] bg-white p-4 transition-all hover:shadow-md group">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <span className="mt-2 inline-flex rounded-full bg-blue-50 text-[#0074E0] px-2 py-1 text-[9px] font-black uppercase">{service.serviceType}</span>
                      </div>
                    </div>
                  </button>
                ))}
            ...
            <section id="profile-reviews" className="bg-white rounded-3xl border border-gray-200 shadow-sm p-6 sm:p-8 scroll-mt-24">
              <div className="flex items-center gap-2 mb-2">
                <Star className="w-5 h-5 text-[#D32323] fill-[#D32323]" />
                <h2 className="text-xl font-black text-[#333]">Reseñas y Calificaciones</h2>
              </div>
              <p className="text-[10px] uppercase tracking-wider font-black text-gray-400 border-b border-[#333]/30 pb-3">Opiniones de clientes auditados y valoración acumulativa</p>

              <div className="mt-6 rounded-2xl border border-[#333]/50 p-5 grid grid-cols-1 md:grid-cols-[170px_1fr] gap-6 items-center">
                <div className="text-center">
                  <strong className="block text-5xl font-black text-[#0B1F3A]">{currentAgency.rating.toFixed(1)}</strong>
                  {starRow(currentAgency.rating)}
                  <p className="mt-1 text-[10px] font-black text-gray-400 uppercase">({currentAgency.reviewsCount} reseñas totales)</p>
                </div>
                <div className="space-y-2">
                  {ratingBreakdown.map((item) => (
                    <div key={item.rating} className="grid grid-cols-[24px_1fr_42px] items-center gap-3 text-xs font-bold text-gray-500">
                      <span>{item.rating}★</span>
                      <div className="h-2.5 rounded-full bg-gray-200 overflow-hidden"><div className="h-full bg-amber-400" style={{ width: `${item.percent}%` }} /></div>
              ...
              <form onSubmit={submitReview} className="mt-6 rounded-2xl border border-gray-200 bg-white p-5 space-y-4">
                <h3 className="text-xs font-black uppercase tracking-wider text-[#333]">Escribir una valoración de cliente</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <label className="space-y-1">
                    <span className="text-[10px] font-black uppercase text-gray-400">Calificación</span>
                    <select value={reviewRating} onChange={(event) => setReviewRating(Number(event.target.value))} className="w-full rounded-xl border border-[#333]/50 px-4 py-3 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[#D32323]/20">
                  ...
                  <label className="space-y-1">
                    ...
                    <input value={reviewName} onChange={(event) => setReviewName(event.target.value)} placeholder="Ej. Carlos Gómez" className="w-full rounded-xl border border-[#333]/50 px-4 py-3 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[#D32323]/20" />
                  </label>
                </div>
                <label className="block space-y-1">
                  <span className="text-[10px] font-black uppercase text-gray-400">Comentario</span>
                  <textarea required value={reviewText} onChange={(event) => setReviewText(event.target.value)} rows={4} placeholder="Describe brevemente el éxito o tu grado de satisfacción con el posicionamiento local..." className="w-full rounded-xl border border-[#333]/50 px-4 py-3 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#D32323]/20" />
                </label>
                <button type="submit" className="w-full rounded-xl bg-[#D32323] hover:bg-[#b01c1c] text-white py-3 text-xs font-black uppercase tracking-wider">Publicar Reseña en la Ficha</button>
                {reviewNotice && <p className="text-xs font-bold text-emerald-700 bg-emerald-50 rounded-xl px-4 py-3">{reviewNotice}</p>}
              </form>
                  ...
                  <div key={review.id} className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-blue-50 text-[#0074E0] flex items-center justify-center text-xs font-black">{review.author.charAt(0)}</div>
                        <div>
                          <h4 className="text-sm font-black text-[#333]">{review.author}</h4>
                          {starRow(review.rating, 'w-3.5 h-3.5')}
                        </div>
                      </div>
                      <div className="text-right text-[10px] font-bold text-gray-400">
                        <p>{review.createdAt || 'Reciente'}</p>
                        {review.city && <p>Sede: {review.city}</p>}
                      </div>
                    </div>
                    <p className="mt-3 text-xs font-semibold text-gray-600 leading-relaxed">“{review.body}”</p>
                  </div>
                ))}
              </div>
            </section>
