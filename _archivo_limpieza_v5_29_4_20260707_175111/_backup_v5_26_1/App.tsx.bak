...
import MarketplaceDynamics from './components/MarketplaceDynamics';
...
export default function App() {
  const getPageFromHash = (): AppPage => {
    ...
    if (window.location.hash.startsWith('#/agencias/')) return 'agencyProfile';
  ...
  useEffect(() => {
    document.title = currentPage === 'searchResults'
      ? 'Resultados de búsqueda | SEOLOCAL'
      : currentPage === 'agencyProfile'
      ? 'Perfil de agencia SEO Local | SEOLOCAL'
  ...
  const navigateToSearchResults = (keyword = '', location = '') => {
    ...
    window.location.hash = `/buscar${params.toString() ? `?${params.toString()}` : ''}`;
  };

  const getSearchParamsFromHash = () => {
    const raw = window.location.hash.replace('#/buscar', '').replace(/^\?/, '');
    const params = new URLSearchParams(raw);
    return {
      query: params.get('q') || searchState.keyword || '',
      location: params.get('loc') || searchState.location || '',
    };
  };
  ...
  const navigateToAgencyProfile = (agency: Agency) => {
    window.location.hash = `/agencias/${agency.slug || agency.id}`;
  };
  ...
  const handleHeroSearch = (keyword: string, location: string) => {
    ...
    navigateToSearchResults(cleanKeyword, cleanLocation);
  };
  ...
  const selectedAgencyProfileIdentifier = currentPage === 'agencyProfile'
    ? decodeURIComponent(window.location.hash.replace('#/agencias/', '').split(/[?#]/)[0] || '')
    : '';
  const selectedAgencyProfile = selectedAgencyProfileIdentifier
    ? agenciesList.find((agency) => agency.slug === selectedAgencyProfileIdentifier || agency.id === selectedAgencyProfileIdentifier || agency.id === `agency-${selectedAgencyProfileIdentifier}`)
    : undefined;
  ...
  const searchPageParams = currentPage === 'searchResults'
    ? getSearchParamsFromHash()
    : { query: searchState.keyword, location: searchState.location };
      ...
      <main className="flex-1">
        ...
        {currentPage === 'searchResults' && (
          <SearchResultsPage
            ...
            categories={marketplaceCategories}
            services={servicesList}
            query={searchPageParams.query}
            location={searchPageParams.location}
            onSelectAgency={navigateToAgencyProfile}
            onOpenService={navigateToService}
            onSelectCategory={handleDirectoryCategorySelect}
            onBackHome={() => navigateToHome()}
            onSearchAgain={handleHeroSearch}
          />
        )}
        ...
        {currentPage === 'agencyProfile' && (
          <AgencyProfilePage
            ...
            profileIdentifier={selectedAgencyProfileIdentifier}
            favorites={favorites}
            servicesCatalog={servicesList}
            onToggleFavorite={handleToggleFavorite}
            onBackToDirectory={navigateToAgencies}
            onRequestQuote={handleHireAgency}
            onOpenService={navigateToService}
